...
(() => {
  ...
  function drawEvent(ctx,e,x,y,w,h,kind){
    ...
    if(kind==='team' && e.show_label!==false && e.label){
      const nameSize=fitFont(ctx,p.name,nameMax,30,22,700);
      const labelSize=fitFont(ctx,e.label,nameMax,17,13,500);
  ...
  async function exportClean(kind){
    ...
    for(let idx=0;idx<42;idx++){
      ...
      const gap=7;
      const ideal=kind==='team'?58:52;
      let rowH=Math.min(ideal,(available-gap*(evs.length-1))/evs.length);
      rowH=Math.max(kind==='team'?40:38,rowH);
  ...
  if(badge)badge.textContent='v2.9';
})();